export const ROUTES = {
  MAIN_GET_SECTIONS: '/api/main/get=sections',

  STAFF_ADD_SECTION: '/api/main/add=section',
  STAFF_DELETE_SECTION: '/api/main/delete=section',
  STAFF_DELETE_THEME: '/api/main/delete=theme',
  STAFF_ADD_THEME: '/api/main/add=theme',
  STAFF_UPDATE_THEME: '/api/main/update=sections_theme',
  STAFF_UPDATE_SECTION: '/api/main/update=section',
  STAFF_GET_ELEMENTS: '/api/main/get=sections_elements',
  STAFF_GET_SELECTED_THEME: '/api/main/get=selected_theme',
  STAFF_GET_SELECTED_ELEMENT: '/api/main/get=selected_element',
  STAFF_CHECK_ADDING_ROLE: '/staff/checkAddingRole',
  STAFF_GET_USER_TO_LIST: '/staff/updateRoles',
  STAFF_GET_USERS_LIST: '/staff/getUsersList',
  STAFF_GET_USER: '/staff/getUser',

  GET_PROFILE_USER: '/staff/getUser',

  AUTH_IS_LOGGED: '/login/check/authorize',
  AUTH_IS_VALID_EMAIL: '/login/check/email',
  AUTH_ADD_NEW_USER: '/login/add_user',
  AUTH_LOGINING: '/login/auth',
  AUTH_LOGOUT: '/logout',
};
